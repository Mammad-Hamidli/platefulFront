...
export const WaiterOrderDetail: React.FC = () => {
  const { orderId } = useParams<{ orderId: string }>();
  const { order, isLoading } = useOrder(Number(orderId));
  const { serveOrder } = useWaiterOrders();
  ...
  const canServe = order.status === 'PREPARED_WAITING';
      ...
      <div className="space-y-3">
        {canServe && (
          <Button
            ...
            size="lg"
            className="w-full"
            onClick={() => serveOrder(order.id, { onSuccess: () => navigate('/waiter') })}
          >
            Mark as Served
          </Button>
        )}
        {canServe && (
          <Button
            ...
            size="lg"
            className="w-full"
            onClick={() => navigate(`/waiter/payment/${order.id}`)}
          >
            Process Payment
          </Button>
        )}
